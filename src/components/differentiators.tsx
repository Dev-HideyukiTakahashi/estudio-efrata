      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, index) => (
            ...
            >
              <div
                className={`relative p-6 rounded-2xl backdrop-blur-sm border ${item.borderColor} bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-500`}
              >
                <div
                  className={`w-12 h-12 rounded-xl bg-gradient-to-br ${item.color} flex items-center justify-center mb-4 ${item.textColor}`}
                >
                  <item.icon size={22} />
                </div>
